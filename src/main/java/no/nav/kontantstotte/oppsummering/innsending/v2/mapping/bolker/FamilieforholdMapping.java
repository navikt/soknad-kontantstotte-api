...
import static no.nav.kontantstotte.oppsummering.innsending.v2.mapping.Tekstnokkel.SVAR_JA;
import static no.nav.kontantstotte.oppsummering.innsending.v2.mapping.Tekstnokkel.SVAR_NEI;
...
public class FamilieforholdMapping implements BolkMapping {
    ...
    public Bolk map(Soknad soknad, Map<String, String> tekster, Unleash unleash) {
        ...
        if("NEI".equalsIgnoreCase(familieforhold.borForeldreneSammenMedBarnet)){
            bolk.elementer.add(Element.nyttSvar(tekster.get(FAMILIEFORHOLD_BOR_SAMMEN), tekster.get(SVAR_NEI.getNokkel())));
        }
        if("JA".equalsIgnoreCase(familieforhold.borForeldreneSammenMedBarnet)){
            bolk.elementer.add(Element.nyttSvar(tekster.get(FAMILIEFORHOLD_BOR_SAMMEN), tekster.get(SVAR_JA.getNokkel())));
